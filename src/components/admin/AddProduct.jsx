...
const AddProduct = ({ setIsProductModal }) => {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      if (res.status === 200) {
        setIsProductModal(false), toast.success("Product added successfully");
      }
    } catch (error) {
      console.log(error);
    }
              ...
              <div className="mt-2 flex  gap-2">
                {extraOptions.map((item, index) => (
                  <span
                    ...
                    onClick={() => {
                      setExtraOptions(
                        extraOptions.filter((i) => i.text !== item.text)
                      );
                    }}
                    className="inline-block border cursor-pointer border-orange-500 text-orange-500  p-1 rounded-xl text-xs"
                  >
                    {item.text}
                  </span>
                ))}
